          ...
          if (doInitWebSocket) {
            if (wsSkt[index] == null && wsServers[index] != null) {
              ...
              wsSkt[index].onopen = function(event) {
                ...
                console.log("WS Connect to " + wsServers[index] + " on " + index);
              ...
              wsSkt[index].onerror = function(error) {
                console.log("WS Error: " + error.reason);
              }
              wsSkt[index].onclose = async function(event) {
                await sleep(200);
                console.log("WS Disconnected " + wsServers[index] + " on " + index + " : " + event.code + ' - ' + event.reason);
                wsSkt[index] = null;
                // event.codes:
       ...
       function sendWsMsg(msg, index = 0) {
        ...
        function showLog(reqStr, fromUser = true) {
          const date = new Date();
          // add timestamp to received text if generated by browser (24h HH:MM:SS.mmm to match device log format)
          const p2 = (n) => String(n).padStart(2, '0');
          const p3 = (n) => String(n).padStart(3, '0');
          const ts = p2(date.getHours()) + ':' + p2(date.getMinutes()) + ':' + p2(date.getSeconds()) + '.' + p3(date.getMilliseconds());
          let logText = fromUser ? "[" + ts + " Web] " : "";
          logText += reqStr;
        ...
        async function getConfig(cfgGroup) {
          ...
          if (response.ok) {
            ...
            buildTable(configData, cfgGroup); // format received json into html table with data
                ...
                if (nextPair == 0) {
                  ...
                  switch (value.charAt(0)) {
                    case 'A': // action button
                      inputHtml = '<button type="button" class="action-btn" id="' + saveKey + '" value="1">' + saveVal + '</button>';
                    break;
